...
Queue.setMethod(function push(nr, value) {
	...
	if (this._prev_received + 1 !== nr) {
		...
		// Get the previous request nr
		prev_request_nr = this._prev_request;

		// Calculate the difference, multiple numbers could be missing
		diff = nr - prev_request_nr - 1;

		console.log('');
		console.log('Received nr: ', nr);
		console.log('But previous request was', prev_request_nr);
		console.log('Need to request', diff, 'nrs:');
		...
		if (diff > 0) {
			...
			console.log('Tried to request negative nrs:', diff);
			console.log('^^^');
		}
...
Queue.setMethod(function _continue() {
	...
	while (this._queue.length) {
		...
		if (entry.nr == this._prev_forward + 1) {
			...
			console.log('Flushing nr', entry.nr);
...
Queue.setMethod(function request(nr) {
	...
	if (this._request_fnc) {
		...
		this._request_fnc(nr, function gotValue(err, val) {
			...
			if (timedout) {
				console.log('Request response came too late', nr);
				return;
			}
		...
		timer = setTimeout(function waitForTimeout() {
			...
			that.emit('timeout', nr, Date.now() - start);
