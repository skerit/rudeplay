			...
			case 84:
				...
				if (!req.session.initial_rtp_timestamp) {
					...
					if (Blast.DEBUG) {
						...
						Rudeplay.log('Initial RTP timestamp not yet set, not setting next sequence!');
					}
				...
				if (next_seq < 0) {
					if (Blast.DEBUG) {
						Rudeplay.log('Next sequence to play is smaller than 0:', next_seq);
					}

					return;
				}

				req.session.rtsp_sequence._next_forward_seq = next_seq % 65535;

				Rudeplay.log('Next sequence to play:', next_seq);

				Rudeplay.log('NOW-Latency:', now_minus_latency, 'Last sync:', time_last_sync, time_last_sync_frac, 'NOW:', now);
