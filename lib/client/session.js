...
var Session = Fn.inherits('Informer', 'Develry.Rudeplay.Client', function Session(device) {
	...
	setInterval(function() {
		...
		var packet = that.packet_buffer.readPacket();

		if (packet == null) return;

		that.encoder.write(packet.pcm);
	}, 7);
	...
	this.on('ready', function onReady() {
		ready = true;
		that.play_start = that.convertUnixToRtp();
	});
});
 ...
 * @return   {Number}
 */
Session.setMethod(function convertUnixToRtp(timestamp) {
	...
	result = Rudeplay.low32(~~(Date.now() / 1000));
...
Session.setMethod(function createOutgoing(method, uri, callback) {
	...
	if (callback) {
		out.once('response', function onResponse(res) {
			...
			console.log('GOT RESPONSE TO', method, out, res);
