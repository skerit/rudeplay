 ...
 * @version       0.1.0
 */
var Incoming = Fn.inherits(stream.PassThrough, 'Develry.Rudeplay', function IncomingMessage(connection, header) {
	...
	this.res = new Rudeplay.OutgoingMessage(this);
...
Incoming.setMethod(function createOrGetSession() {
	...
	session = Rudeplay.Session.getSession(this);

	console.log('Attempted to get existing session:', session);

	if (session == null) {
		console.log('Could not find session, creating new one for', this);
		session = new Rudeplay.Session(this);
...
Incoming.setMethod(function _processHeader(header) {
	...
	for (i = 0; i < lines.length; i++) {
		...
		if (name) {
			addHeaderLine(name, val, headers);
		}
