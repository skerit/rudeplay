	...
	this.socket.on('data', function onData(chunk) {
		...
		if (processed < content_length) {
			...
			if (processed + chunk.length > content_length) {
				...
				if (temp.length > 10) {
					console.log('Got extra body: "' + temp + '"');
					console.log('__ LAST CHUNK: ' + chunk.toString('utf-8'));

					// And trim the chunk, too
					chunk = chunk.slice(0, index);

					console.log('Expected length was', content_length);
					console.log('Processed length is now', chunk.length + processed);
