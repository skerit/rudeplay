...
Server.setMethod(function broadcast() {
	...
	if (Blast.DEBUG) {
		console.log('Starting new Rudeplay server: ', JSON.stringify(this.name));
	}
...
Server.setMethod(function setupAction(req, res) {
	...
	if (req.headers.session) {
		...
		if (Blast.DEBUG) {
			console.log('SETUP method: disallowing aggregate operation');
		}
	...
	if (Blast.DEBUG) {
		console.log('------------')
		console.log('CLIENT TRANSPORT:', req.session.client_transport);
		console.log('------------')
	}
...
Server.setMethod(function setParameterAction(req, res) {
	...
	req.getBody(function gotBody(err, body) {
		...
		switch (content_type) {
			...
			default:
				console.log('Unknown SET_PARAMETER method:', body);
		}
