...
// Disable the compatibility warnings
process.env.AVAHI_COMPAT_NOWARN = 1;

try {
	mdns = require('mdns');

	// Mdns always shows another warning, if janeway is loaded
	// it will need to refersh the screen
	if (typeof __Janeway != 'undefined' && __Janeway.redraw) {
		__Janeway.redraw();
	}
} catch (err) {
	console.log('RUDEPLAY: mdns module not found, announcements will not work');
}
 ...
 * @version  0.2.0
 */
var Server = Fn.inherits('Informer', 'Develry.Rudeplay.Server', function Server(settings) {
	...
	this._server.on('connection', function gotConnection(socket) {
		...
		var conn = new Rudeplay.Server.Connection(that, socket);
	...
	// Start the server
	this._server.listen(this.port, function isListening() {

		if (Blast.DEBUG) {
			Rudeplay.log('Server "' + that.name + '" is listening on port', that.port);
		}
...
/**
 * Expose the mdns instance, if it's available
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    0.1.3
 * @version  0.1.3
...
Server.setProperty('default_settings', {
	...
	// The identification name of this server
	name               : 'Rudeplay Server',

	// The optional version of this server
	version            : '1.0.0',

	// The mac address (auto-generated if empty)
	mac                : '',

	// The port this server listens on
	port               : 5000,

	// Time in ms to wait for retransmissions
	retransmit_timeout : 500,

	// Use software volume setting
...
Server.setMethod(function processAirplay(req, res) {
	...
	if (Blast.DEBUG) {
		Rudeplay.log('Received request:', req.method);
	}
 ...
 * the actual servers are created here
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    0.1.0
 * @version  0.1.0
...
Server.setMethod(function setupAction(req, res) {
	...
	req.session.client_transport = req.splitHeaderLine(req.headers['transport']);
	...
	Fn.parallel(function rtp(next) {
		...
		var server = new Rudeplay.Server.RtpServer(req);
		...
		var server = new Rudeplay.Server.RtpControlServer(req);
		server.once('done', next);

	}, function timing(next) {

		var server = new Rudeplay.Server.TimingServer(req);
		server.once('done', next);

	}, function setAlac(next) {

		req.session.createDecoder();
...
Server.setMethod(function announceAction(req, res) {
	...
	req.getBody(function gotBody(err, buffer) {
		...
		if (conf.media[0].fmtp[0]) {
			...
			req.session.codec = 'alac';
		} else {
			req.session.codec = 'pcm';
		}
 ...
 * This should stop the streaming
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    0.1.0
 * @version  0.1.0
...
Server.setMethod(function flushAction(req, res) {
	...
	// @TODO: fix this value, this should probably
	// be the last played rtp timestamp?
...
Server.setMethod(function recordAction(req, res) {
	...
	// This is hardcoded to 2205, like in apple devices
...
Server.setMethod(function teardownAction(req, res) {
	...
	// Teardown means: reset the session
	req.session.reset();
 ...
 * @version  0.1.3
 */
Server.setMethod(function destroy() {
	...
	if (this.mdns_instance) {
		this.mdns_instance.stop();
	}
...
// If Janeway has loaded, use print as the logger
if (typeof __Janeway !== 'undefined') {
	Rudeplay.log = function log() {
		__Janeway.print('info', arguments, {level: 1});
	}
}
