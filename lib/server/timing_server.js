 ...
 */
var Timing = Fn.inherits('Informer', 'Develry.Rudeplay.Server', function TimingServer(req) {
	...
	this.server.on('message', function gotRtpMessage(msg, rinfo) {
		...
		if (Blast.DEBUG) {
			Rudeplay.log('Got Timing message: ' + msg);
		}
	});
	...
	this.server.bind(function bound() {
		...
		if (Blast.DEBUG) {
			Rudeplay.log('Timing server listening', addr.port);
		}
