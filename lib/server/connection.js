 ...
 */
var Connection = Fn.inherits('Informer', 'Develry.Rudeplay.Server', function Connection(rudeplay, socket) {

	// Store the main server instance
	if (rudeplay instanceof Rudeplay.Server.Server) {
		this.rudeplay = rudeplay;
	} else {
		this.client_session = rudeplay;
	}
...
Connection.setMethod(function _processSocket() {
	...
	this.socket.on('data', function onData(chunk) {
		...
		if (!received_header) {
			...
			req = new Rudeplay.Server.IncomingMessage(that, header);
