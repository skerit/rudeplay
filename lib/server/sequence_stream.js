 ...
 */
var RtspSequence = Fn.inherits(stream.Readable, 'Develry.Rudeplay.Server', function RtspSequenceStream(session, options) {
	...
	this._sequeue = new Rudeplay.Server.Queue();

	// When was the last update to next_forward_seq
	this._forward_seq_last_update = 0;
...
RtspSequence.setMethod(function setNextSequence(seq) {
	...
	// When was this set last?
	this._forward_seq_last_update = Date.now();
...
RtspSequence.setMethod(function _drainQueue() {
	...
	if (Blast.DEBUG) {
		Rudeplay.log('Draining', this._queuex.length, 'greater than', this._next_forward_seq);
	}
